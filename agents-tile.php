...
if ($result1->execute()) {
$num_rows1 = $result1->fetch(PDO::FETCH_NUM);
...
/* Otherwise display table of all agents in firm and office */
$cell_num = 1;
while ($row1 = $result1->fetch(PDO::FETCH_ASSOC)) {
      if ($cell_num == 1) {
         echo " <tr>\n";
      ...
      if (agent_photo_exists($row1[$db_agent_id])) {
         $photo_src = agent_photo_exists($row1[$db_agent_id]);
         $img_info = getimagesize($abs_agents.$photo_src);
         echo "<a href='".$_SERVER['PHP_SELF']."?agent_id=".$row1[$db_agent_id]."&$str_url_vars'><img src='".$http_agents.$photo_src."' ".$img_info[3]." alt='".$row1[$db_agent_fname]." ".$row1[$db_agent_lname]."' border='0' /></a><br />\n";
      }
      echo "<a href='".$_SERVER['PHP_SELF']."?agent_id=".$row1[$db_agent_id]."&$str_url_vars'><strong>".$row1[$db_agent_fname]." ".$row1[$db_agent_lname];
      echo "</strong></a><br />\n";
      if (!empty($row1[$db_agent_email])) {
      	 echo "<a href='mailto:".$row1[$db_agent_email]."'>Contact Me</a><br />\n";
      }
...
if ($cell_num < $columns) {
   echo "  <td colspan='".($columns - $cell_num)."'>&nbsp;</td>\n</tr>\n";
}

}
echo "</table><br />\n";
} else {
   echo "Error performing query: " . getMessage() . "<br />\n";
   echo "sql1: $sql1<br />\n";
}

...
/* Select waterfront names to be used 
$sql_Waterfront = "SELECT DISTINCT $db_Waterfront FROM $db_tbl_residential ORDER BY $db_Waterfront";
$result_Waterfront = $dbcnx->prepare($sql_Waterfront);
$result_Waterfront->execute();
$data_waterfront = $result_Waterfront->fetchAll(PDO::FETCH_ASSOC);*/
...
/*if (!empty($db_Waterfront)) {
   echo "<option value='$db_Waterfront'>$db_Waterfront</option>\n";
}
echo "<option value=''>---</option>\n";
foreach($data_waterfront as $row_waterfront) {
   echo "<option value='".$row_waterfront[$db_Waterfront]."'>".$row_waterfront[$db_Waterfront]."</option>\n";
}*/
 if (!empty($waterfront)) {
   /*Let's make it look nice */
   if ($waterfront == 'Y') {
...
if (!empty($mls_id)) {
   ...
   if (!empty($waterfront)) {
      if ($where_clause_num > 0) { $where_clause .= " AND"; }
		  if ($waterfront == 'Y') {
			  $where_clause .= " $db_Waterfront :IS NOT NULL";
			  $where_clause_arr['waterfront'] = $waterfront;
		  }else{
			  $where_clause .= " $db_Waterfront :IS NULL";
			  $where_clause_arr['waterfront'] = $waterfront;
		  }
		  $where_clause_num++;
   }
